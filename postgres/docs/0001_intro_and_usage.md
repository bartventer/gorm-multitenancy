...
[![Go Reference](https://pkg.go.dev/badge/github.com/bartventer/gorm-multitenancy.svg)](https://pkg.go.dev/github.com/bartventer/gorm-multitenancy/postgres/v7)
...
Call [`RegisterModels`](https://pkg.go.dev/github.com/bartventer/gorm-multitenancy/postgres/v7#RegisterModels) after initializing the database to register all models.
...
Alternatively, you can pass the models as variadic arguments to [`postgres.New`](https://pkg.go.dev/github.com/bartventer/gorm-multitenancy/postgres/v7#New) when creating the dialect.
...
Or pass the models as variadic arguments to [`postgres.Open`](https://pkg.go.dev/github.com/bartventer/gorm-multitenancy/postgres/v7#Open) when creating the dialect.
...
Call [`MigratePublicSchema`](https://pkg.go.dev/github.com/bartventer/gorm-multitenancy/postgres/v7#MigratePublicSchema) to create the public schema and migrate all public models.
...
Call [`CreateSchemaForTenant`](https://pkg.go.dev/github.com/bartventer/gorm-multitenancy/postgres/v7#CreateSchemaForTenant) to create the schema for a tenant and migrate all tenant-specific models.
...
Call [`DropSchemaForTenant`](https://pkg.go.dev/github.com/bartventer/gorm-multitenancy/postgres/v7#DropSchemaForTenant) to drop the schema and cascade all schema tables.
...
You can embed the [postgres.TenantModel](https://pkg.go.dev/github.com/bartventer/gorm-multitenancy/postgres/v7#TenantModel) struct in your tenant model to add the necessary fields for the tenant model.
...
Use the [`WithTenantSchema`](https://pkg.go.dev/github.com/bartventer/gorm-multitenancy/postgres/v7/scopes#WithTenantSchema) scope function when you want to perform operations on a tenant specific table, which may include foreign key constraints to a public schema table(s).
...
Use the [`SetSearchPath`](https://pkg.go.dev/github.com/bartventer/gorm-multitenancy/postgres/v7/schema#SetSearchPath) function when the tenant schema table has foreign key constraints you want to access belonging to other tables in the same tenant schema (and or foreign key relations to public tables).
